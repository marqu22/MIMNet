...
# 初始化uid_embedding ,iid_embedding
...
class MetaNet(torch.nn.Module):
    ...
    def forward(self, rsc_inter_seq_emb, tgt_inter_seq_emb, seq_index, aim_domin_seq_inter=None, traing_mode=True):  ##tgt_目标域交互信息,seq_index源域交互信息
        if traing_mode == True:
            ...
            return (orgin_output.squeeze(1), orgin_output.squeeze(1))
...
class CapsuleBridge(torch.nn.Module):
    ...
    def disgreement_loss(t1, t2):
        ...
        * @return disagreement_loss
        ...
        div = torch.bmm(t1_L2, t2_L2.transpose(-1, -2))
        loss = simil / torch.max(div, torch.ones_like(div) * 1e-5)
        int_mask = torch.tril(torch.ones((t1.shape[0], t1.shape[1], t1.shape[1])), diagonal=-1)
        print(int_mask)
        print(loss.shape)
        bool_mask = int_mask == 1
        loss = torch.masked_select(loss, bool_mask)
        return loss
    ...
    def forward(self, rsc_inter_seq_emb, tgt_inter_seq_emb, seq_index, aim_domin_seq_inter=None, traing_mode=True):  ##tgt_目标域交互信息,seq_index源域交互信息
        ...
        if traing_mode == True:
             ## add:
            one_mask = torch.zeros(seq_index.shape[0],20,).to(device="cuda:0")
            one_mask[:,0]=1
            one_mask=torch.where(one_mask==1,0,1)
            mask = one_mask  ## 0表示不mask
            ...
            # drop out method
            dropout_rate = 0.5
            drop_mask_1 = torch.bernoulli(torch.where(mask == 1, 1.0, dropout_rate)).to(torch.int)  ## 0表示不mask
            gen_seq_len = torch.where(drop_mask_1 == 1, 0, 1).sum(dim=1)
            gen_seq_len = gen_seq_len.unsqueeze(1)
            multi_interset_gen = self.capsuleLayer(rsc_inter_seq_emb, gen_seq_len)
            # disagree_loss = self.disgreement_loss(multi_interset, multi_interset)
            # note: behavior_embs: [N, L, D] seq_len: [N, 1]
            # note: return : B * max_interest* dim
            # orgin_output = self.decoder_simple(multi_interset)
            gen_output = self.decoder(multi_interset_gen)

            return orgin_output.squeeze(1), gen_output.squeeze(1), disagree_loss
...
class MFBasedModel(torch.nn.Module):
    def __init__(self, uid_all, iid_all, num_fields, emb_dim, meta_dim_0, K=512, interest_num=3, prot_alpha=0.5, wo_att=0, wo_capsule=0, wo_mutli_inter=0, wo_att_proj=0):
        ...
        # self.capsule_mapping_cross_user_embedd_Q = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim))
        ...
        # self.capsule_mapping_cross_user_embedd_proj = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim))

        # self.capsule_mapping_cross_user_embedd_prot_Q = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim))
        self.capsule_mapping_cross_user_embedd_prot_K = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim))
        self.capsule_mapping_cross_user_embedd_prot_V = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim))
        # self.capsule_mapping_cross_user_embedd_prot_proj = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim))
        self.dynamic_fusion = torch.nn.Sequential(torch.nn.Linear(emb_dim * 2, 1), torch.nn.Sigmoid())
    ...
    def cal_c_loss(self, pos, aug, tau):
        ...
        pos_score = torch.exp(pos_score / tau)  # [batch_size]
        ttl_score = torch.sum(torch.exp(ttl_score / tau), axis=1)  # [batch_size]
    ...
    def forward(self, x, stage, tau=0.5):
        ...
        if stage == "train_src":
            ...
            x = torch.sum(emb[:, 0, :] * emb[:, 1, :], dim=1)  # [256, 10]*(256,10),这里不是要256个用户对256个item的评分(256,256),而是对指定的item的评分(256,2)
            return x
        ##
        ## note:meta_train_stage
            ...
            ##note: 获取基本的内容
            iid_emb = self.tgt_model.iid_embedding(x[:, 1].unsqueeze(1))  ## 获得目标域item embedding
            tgt_iid_emb_prototype = self.tgt_iid_cluser_prototypes_embedding[self.tgt_iid_cluser_labels[x[:, 1]]].unsqueeze(dim=1)
            # src_iid_emb = self.src_model.iid_embedding(x[:, 3].unsqueeze(1))  ## 获得源域item embedding ##没有使用
            uid_emb_src = self.src_model.uid_embedding(x[:, 0].unsqueeze(1))  ## 获得源域 user embedding
            uid_emb_tgt = self.tgt_model.uid_embedding(x[:, 0].unsqueeze(1))  ## 获得目标域域 user embedding
            rsc_inter_seq_emb = self.src_model.iid_embedding(x[:, 2:22])  ## 获得源域交互历史item的embedding
            tgt_inter_seq_emb = self.tgt_model.iid_embedding(x[:, 22:42])  ## 获得目标域交互历史item的embedding
            ###
            ...
            multi_interset_capsule_mapping, multi_interset_capsule_mapping_gen, disagree_loss = self.capsule_bridge.forward(
                rsc_inter_seq_emb, tgt_inter_seq_emb, x[:, 2:]
            )  ##源域交互embedding seq ,目标域交互embedding seq, 和之前两个的总和=> orgin_mapping, gen_mapping

            ##
            ##  test: add: disagreement loss
            # multi_interset_capsule_mapping_T = multi_interset_capsule_mapping.transpose(-1, -2)
            # Ld = torch.bmm(multi_interset_capsule_mapping, multi_interset_capsule_mapping_T)
            # Norm_1 = torch.norm(multi_interset_capsule_mapping, p=1, dim=-1).unsqueeze(dim=-1)
            # Norm_1_T = Norm_1.transpose(-1, -2)
            # norm_all = torch.bmm(Norm_1, Norm_1_T)
            # Ld_f = Ld * (1 / norm_all)
            # disgree_loss = Ld_f.mean(dim=-1).mean(dim=-1).mean(dim=-1)
            ##  test: add: disagreement loss
            ...
            expanded_uid_emb_src = uid_emb_src.expand(size=(multi_interset_capsule_mapping.shape[0], multi_interset_capsule_mapping.shape[1], self.emb_dim)).unsqueeze(dim=2)
            ...
            # ##  test: add: disagreement loss
            # multi_uid_emb_capsule_T = multi_uid_emb_capsule.transpose(-1, -2)
            # Ld = torch.bmm(multi_uid_emb_capsule, multi_uid_emb_capsule_T)
            # Norm_1 = torch.norm(multi_uid_emb_capsule, p=1, dim=-1).unsqueeze(dim=-1)
            # Norm_1_T = Norm_1.transpose(-1, -2)
            # norm_all = torch.bmm(Norm_1, Norm_1_T)
            # Ld_f = Ld * (1 / norm_all)
            # disgree_loss = Ld_f.mean(dim=-1).mean(dim=-1).mean(dim=-1)
            ...
            ## add: tgt 原型
            mapped_multi_uid_emb_capsule_prot_K = self.capsule_mapping_cross_user_embedd_prot_K(multi_uid_emb_capsule)
            mapped_ulti_uid_emb_capsule_prot_V = self.capsule_mapping_cross_user_embedd_prot_V(multi_uid_emb_capsule)
            multi_uid_emb_capsule_prot_T = mapped_multi_uid_emb_capsule_prot_K.transpose(-1, -2)  ## shape(B,10,max_interest)
            coarse_att_prot = torch.bmm(tgt_iid_emb_prototype, multi_uid_emb_capsule_prot_T)  ## shape(B,1,max_interest)
            att_prot = torch.softmax(coarse_att_prot, dim=-1)
            ##      note: 基于多胶囊构造的输出 output_capsule 和 胶囊桥目标域user_embedding:uid_emb_capsule
            uid_emb_capsule_prot = torch.bmm(att_prot, mapped_ulti_uid_emb_capsule_prot_V)  ## shape(B,1,10)

            ## 对比损失

            ### 对比损失另一个视图构造
            multi_interset_capsule_mapping_gen = multi_interset_capsule_mapping_gen.view(multi_interset_capsule_mapping.shape[0], -1, self.emb_dim, self.emb_dim)  ##
            ##      note:  expanded_uid_emb_src被处理为 (B* max_interest* 1 * emb_dim)
            expanded_uid_emb_src = uid_emb_src.expand(size=(multi_interset_capsule_mapping_gen.shape[0], multi_interset_capsule_mapping_gen.shape[1], self.emb_dim)).unsqueeze(dim=2)

            ##      note:  multi_uid_emb_capsule: shape(128,3,10)
            multi_uid_emb_capsule_gen = torch.matmul(expanded_uid_emb_src, multi_interset_capsule_mapping_gen).squeeze(dim=2)  ## Fui的位置
            ##
            ...
            ##      做一个全连接映射
            mapped_multi_uid_emb_capsule_gen_K = self.capsule_mapping_cross_user_embedd_K(multi_uid_emb_capsule_gen)
            mapped_ulti_uid_emb_capsule_gen_V = self.capsule_mapping_cross_user_embedd_V(multi_uid_emb_capsule_gen)

            # mapped_ulti_uid_emb_capsule_V=mapped_multi_uid_emb_capsule_K
            ##      note: attention 融合
            multi_uid_emb_capsule_gen_T = mapped_multi_uid_emb_capsule_gen_K.transpose(-1, -2)  ## shape(B,10,max_interest)
            coarse_att_gen = torch.bmm(iid_emb, multi_uid_emb_capsule_gen_T)  ## shape(B,1,max_interest)
            att_gen = torch.softmax(coarse_att_gen, dim=-1)
            ##      note: 基于多胶囊构造的输出 output_capsule 和 胶囊桥目标域user_embedding:uid_emb_capsule
            uid_emb_capsule_gen = torch.bmm(att_gen, mapped_ulti_uid_emb_capsule_gen_V)  ## shape(B,1,10)

            emb_fuse_gen = torch.cat([uid_emb_capsule_gen, iid_emb], 1)  # 原始生成的目标域embed , 要测试的目标域embed
            output_gen = torch.sum(emb_fuse_gen[:, 0, :] * emb_fuse_gen[:, 1, :], dim=1)  ##
            ...
            cl_loss = self.cal_c_loss(uid_emb_capsule, uid_emb_capsule_gen, tau=tau)
            ...
            alpha = self.dynamic_fusion(torch.concat([iid_emb, tgt_iid_emb_prototype], dim=-1))
            uid_emb_fuse = (1 - alpha) * uid_emb_capsule + (alpha) * uid_emb_capsule_prot
        ...
        ## note:meta_test_stage
            ...
            ### add: capsule bridge; aim :得到胶囊桥映射 multi_interset_capsule_mapping  B * max_interest* (emb_dim*emb_dim)
            multi_interset_capsule_mapping = self.capsule_bridge.forward(rsc_inter_seq_emb, tgt_inter_seq_emb, x[:, 2:], traing_mode=False)  ##源域交互embedding seq ,目标域交互embedding seq, 和之前两个的总和=> orgin_mapping, gen_mapping
            ...
            src_emb = self.public_bridge.forward(src_emb)
            tgt_emb = self.tgt_model.uid_embedding(x.unsqueeze(1)).squeeze()
            return src_emb, tgt_emb
        elif stage == "test_map":
            uid_emb = self.public_bridge.forward(self.src_model.uid_embedding(x[:, 0].unsqueeze(1)).squeeze())
            emb = self.tgt_model.forward(x)
            emb[:, 0, :] = uid_emb
            x = torch.sum(emb[:, 0, :] * emb[:, 1, :], dim=1)
            return x
