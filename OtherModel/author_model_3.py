...
class MetaNet(torch.nn.Module):
    ...
    def forward(self, emb_fea, seq_index):
        ...
        event_K = self.event_K(emb_fea) ## event_k表示item对转移共享大小
...
class MFBasedModel(torch.nn.Module):
    ...
    def forward(self, x, stage,tau=0.5):
        if stage == 'train_src':
            ...
            uid_emb_src = self.src_model.uid_embedding(x[:, 0].unsqueeze(1)) ## 获得源域user embedding 
            ufea = self.src_model.iid_embedding(x[:, 2:]) ## 获得交互历史item的embedding
            mapping = self.meta_net.forward(ufea, x[:, 2:]).view(-1, self.emb_dim, self.emb_dim) ## mapping ==Fui
            uid_emb = torch.bmm(uid_emb_src, mapping)  ## Fui的位置
